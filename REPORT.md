...
- The builder now requires a concept anchor near the matched value
  (`concept_supported`): a chunk qualifies only if wording identifying the
  figure — "net sales", "research and development", "total assets" — occurs
  within a few hundred characters of the number. Near, not merely present: a
  10-K page that mentions revenue somewhere also contains dozens of unrelated
  numbers.
